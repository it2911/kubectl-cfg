...
var (
	...
	createAuthInfoExample = templates.Examples(`
		# Set only the "client-key" field on the "AUTHINFO_NAME"
		# entry, without touching other values:
		kubectl cfg add auth AUTHINFO_NAME --client-key=~/.kube/admin.key

		# Set basic auth for the "AUTHINFO_NAME" entry
		kubectl cfg add auth AUTHINFO_NAME --username=admin --password=uXFGweU9l35qcif

		# Embed client certificate data in the "AUTHINFO_NAME" entry
		kubectl cfg add auth AUTHINFO_NAME --client-certificate=~/.kube/admin.crt --embed-certs=true

		# Enable the Google Compute Platform auth provider for the "AUTHINFO_NAME" entry
		kubectl cfg add auth AUTHINFO_NAME --auth-provider=gcp

		# Enable the OpenID Connect auth provider for the "AUTHINFO_NAME" entry with additional args
		kubectl cfg add auth AUTHINFO_NAME --auth-provider=oidc --auth-provider-arg=client-id=foo --auth-provider-arg=client-secret=bar

		# Remove the "client-secret" config value for the OpenID Connect auth provider for the "AUTHINFO_NAME" entry
		kubectl cfg add auth AUTHINFO_NAME --auth-provider=oidc --auth-provider-arg=client-secret-

		# Enable new exec auth plugin for the "AUTHINFO_NAME" entry
		kubectl cfg add auth AUTHINFO_NAME --exec-command=/path/to/the/executable --exec-api-version=client.authentication.k8s.io/v1beta

		# Define new exec auth plugin args for the "AUTHINFO_NAME" entry
		kubectl cfg add auth AUTHINFO_NAME --exec-arg=arg1 --exec-arg=arg2

		# Create or update exec auth plugin environment variables for the "AUTHINFO_NAME" entry
		kubectl cfg add auth AUTHINFO_NAME --exec-env=key1=val1 --exec-env=key2=val2

		# Remove exec auth plugin environment variables for the "AUTHINFO_NAME" entry
		kubectl cfg add auth AUTHINFO_NAME --exec-env=var-to-remove-`)
)
...
func newCmdCfgAddAuthInfo(out io.Writer, options *kconf.CreateAuthInfoOptions) *cobra.Command {
	cmd := &cobra.Command{
		Use: fmt.Sprintf(
			"auth AUTHINFO_NAME [--%v=path/to/certfile] "+
				"[--%v=path/to/keyfile] "+
				"[--%v=bearer_token] "+
				"[--%v=basic_user] "+
				"[--%v=basic_password] "+
